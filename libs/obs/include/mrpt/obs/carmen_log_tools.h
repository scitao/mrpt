...
/** Parse one line from an text input stream and interpret it as a CARMEN log
  ...
  *		- "ROBOTLASER(.*)": A joint odometry-laser entry. This function will
  *return
  *both an mrpt::obs::CObservationOdometry and an
  *mrpt::obs::CObservation2DRangeScan
  *		- "FLASER": (Idem)
  *		- "RLASER": (Idem)
  *		- "PARAM": This case is special (read below).
  *
  *  Note that the sensor position on the robot will be always deduced from the
  *corresponding CARMEN log line automatically.
  *
  *  The following entry types will be IGNORED, since with the tags above will
  *be enough in most applications.
  *		- "ODOM"
  *		- "RAWLASER"
  *
  * About the PARAM entries: This functions has an internal static status
  *consisting on some PARAM values
  *  which have effects on the future read observations. When the function finds
  *one of these, it stores the
  *  parameter value, return an empty vector of observations, and use those
  *values for future laser entries of types FLASER or RLASER.
